...
app.post('/add', function(req, res, next){
	req.assert('name', 'Nome Obbligatorio').notEmpty()           //Validate name
	req.assert('age', 'Età Obbligatoria').notEmpty()             //Validate age
    req.assert('email', 'Indirizzo Email valido obbligatorio').isEmail()  //Validate email
		...
		********************************************/
		...
		req.db.collection('users').insert(user, function(err, result) {
			if (err) {
				...
				req.flash('success', 'Dati Agguinti Correttamente!')
...
app.get('/edit/(:id)', function(req, res, next){
	...
	req.db.collection('users').find({"_id": o_id}).toArray(function(err, result) {
		...
		// if user not found
		if (!result) {
			req.flash('error', 'Utente Non trovato con id = ' + req.params.id)
			res.redirect('/users')
		}
		else { // if user found
...
app.put('/edit/(:id)', function(req, res, next) {
	req.assert('Nome', 'Nome Obbligatorio').notEmpty()           //Validate name
	req.assert('age', 'Età Obbligatoria').notEmpty()             //Validate age
    req.assert('email', 'Indirizzo Email valido obbligatorio').isEmail()  //Validate email
...
app.delete('/delete/(:id)', function(req, res, next) {
	...
	req.db.collection('users').remove({"_id": o_id}, function(err, result) {
		if (err) {
			...
			req.flash('success', 'Utente Eliminato! id = ' + req.params.id)
			// redirect to users list page
			res.redirect('/users')
		}
	})
